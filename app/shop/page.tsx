...
export default function ShopPage() {
  ...
  // Fetch skins — backend returns owned:true/false per skin when authenticated
  ...
  // Re-fetch when wallet connects so owned:true flags update
  ...
  const handleBuy = async (skin: FidgeSkin) => {
    ...
    try {
      ...
      // Update gems from response, mark skin as owned in list
        ...
        <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 14, padding: '14px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            ...
            <p style={{ fontFamily: 'Orbitron,sans-serif', fontWeight: 700, fontSize: 22, color: 'white' }}>
              {loggedIn ? gems : '—'} <span style={{ fontSize: 16 }}>💎</span>
            </p>
          </div>
          <div style={{ textAlign: 'right' }}>
            ...
            <a href="/coming_soon" style={{ display: 'inline-block', marginTop: 8, padding: '6px 14px', background: 'rgba(96,165,250,0.12)', border: '1px solid rgba(96,165,250,0.3)', borderRadius: 8, fontFamily: 'Orbitron,sans-serif', fontSize: 10, fontWeight: 700, color: '#60a5fa', letterSpacing: 1, textDecoration: 'none' }}>
              💎 BUY GEMS →
            </a>
          </div>
        </div>
            ...
            >
              ...
              {skin.is_default ? (
                ...
                <button
                  ...
                  disabled={isBuying || !loggedIn}
                  style={{ width: '100%', padding: '10px', borderRadius: 11, background: canAfford && loggedIn ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.04)', border: `1px solid ${canAfford && loggedIn ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.08)'}`, color: canAfford && loggedIn ? 'white' : 'rgba(255,255,255,0.3)', cursor: isBuying || !loggedIn ? 'default' : 'pointer', fontFamily: 'Orbitron,sans-serif', fontWeight: 700, fontSize: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, transition: 'all .2s' }}
                >
                  {isBuying ? (
