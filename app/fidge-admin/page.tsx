...
const BASE = process.env.NEXT_PUBLIC_API_URL?.replace(/\/$/, '') ?? 'https://backend-production-app.up.railway.app/api';
// ── Types ─────────────────────────────────────────────────────────────────────
...
export default function AdminPage() {
  ...
  const loadPcedoWithdrawals = useCallback(async () => {
    try {
      const res = await fetch(`${BASE}/admin/withdrawals?status=${pcedoWdFilter}`, {
  ...
  const loadGemRequests = useCallback(async () => {
    try {
      const res = await fetch(`${BASE}/admin/gem-requests?status=${gemFilter}`, {
        headers: { 'X-Admin-Token': AdminTokenStore.get() ?? '' },
      });
      const d = await res.json();
  ...
  const handleConfirmWithdrawal = async (id: number) => {
    try {
      const res = await fetch(`${BASE}/admin/withdrawals/${id}/confirm`, {
        method: 'POST',
        headers: { 'X-Admin-Token': AdminTokenStore.get() ?? '' },
      });
      const d = await res.json();
  ...
  const handleVerifyGem = async (id: number) => {
    try {
      const res = await fetch(`${BASE}/admin/gem-requests/${id}/verify`, {
        method: 'POST',
        headers: { 'X-Admin-Token': AdminTokenStore.get() ?? '' },
      });
      const d = await res.json();
  ...
  const handleRejectGem = async (id: number) => {
    try {
      const res = await fetch(`${BASE}/admin/gem-requests/${id}/reject`, {
        method: 'POST',
        headers: { 'X-Admin-Token': AdminTokenStore.get() ?? '' },
      });
