...
export default function LeaderboardPage() {
  ...
  const fetchLb = (showSpinner = false) => {
    if (showSpinner) setRefreshing(true);
    LeaderboardApi.get()
      .then(d => { setLbData(d); setLastUpdated(new Date()); setRefreshIn(REFRESH_SECS); })
      .catch((e) => { setLbError(e instanceof Error ? e.message : 'Failed to load leaderboard'); })
      .finally(() => setRefreshing(false));
  };

  useEffect(() => { fetchLb(); }, []);

  // Auto-refresh every 5 minutes + tick the countdown every second
  useEffect(() => {
    const tick = setInterval(() => {
      setRefreshIn(prev => {
        if (prev <= 1) { fetchLb(); return REFRESH_SECS; }
        return prev - 1;
      });
    }, 1000);
  ...
  // "me" block: backend-computed rank + points, works even outside top 100
  const meData: any = lbData?.me ?? null;
  ...
  // Referral board: sort by referral_count desc, tiebreak by user_id asc (earlier joiner ranks higher)
  const referralBoard = [...rawEntries]
    .sort((a: any, b: any) => {
      const diff = (b.referral_count ?? 0) - (a.referral_count ?? 0);
      return diff !== 0 ? diff : (a.user_id ?? 0) - (b.user_id ?? 0);
    })
    .map((e: any, i: number) => ({ ...e, rank: i + 1 }));

  const board = tab === 'points' ? pointsBoard : referralBoard;

  // For YOU bar — use backend meData (global rank, real points) for points tab.
  // For referrals tab, fall back to board search.
  const userReferralRank = email ? referralBoard.find((e: any) => e.email === email) : null;
  const userRank = tab === 'points'
    ? meData
    : (userReferralRank ?? (meData ? { ...meData, rank: null } : null));
          ...
          <div>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:6, marginTop:8 }}>
              <button
                ...
                disabled={refreshing}
                style={{ background:'none', border:'none', padding:0, cursor:'pointer', display:'flex', alignItems:'center', gap:5, opacity: refreshing ? 0.4 : 1 }}
              >
                <span style={{ fontSize:11, display:'inline-block', animation: refreshing ? '_spin2 0.8s linear infinite' : 'none' }}>⟳</span>
                <span style={{ fontFamily:'Space Mono,monospace', fontSize:8, color:'rgba(255,255,255,0.3)', letterSpacing:'1px' }}>
                  {refreshing ? 'UPDATING…' : `UPDATES IN ${Math.floor(refreshIn / 60)}:${String(refreshIn % 60).padStart(2,'0')}`}
                </span>
              </button>
            </div>
            {lastUpdated && !refreshing && (
              <p style={{ fontFamily:'Space Mono,monospace', fontSize:7, color:'rgba(255,255,255,0.18)', marginTop:3, letterSpacing:'0.5px' }}>
          ...
          <div style={{ position:'fixed', bottom: 100, left:0, right:0, display:'flex', justifyContent:'center', padding:'0 12px', zIndex:500, pointerEvents:'none' }}>
            <div style={{ width:'100%', maxWidth:520, background:'rgba(12,12,12,0.97)', border:'1px solid rgba(255,255,255,0.15)', borderRadius:16, padding:'12px 16px', backdropFilter:'blur(16px)', WebkitBackdropFilter:'blur(16px)', boxShadow:'0 -8px 32px rgba(0,0,0,0.6)', pointerEvents:'all' }}>
              <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                {/* Rank bubble — shows points rank or referral rank depending on active tab */}
                <div style={{ width:38, height:38, borderRadius:'50%', background:'rgba(255,255,255,0.08)', border:'1px solid rgba(255,255,255,0.15)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                  {tab === 'points'
                    ? (meData?.rank
                        ? <span style={{ fontFamily:'Orbitron,sans-serif', fontWeight:700, fontSize: meData.rank >= 1000 ? 8 : meData.rank >= 100 ? 9 : 11, color:'rgba(255,255,255,0.7)', textAlign:'center', lineHeight:1.2 }}>#{meData.rank}</span>
                        : <span style={{ fontSize:16 }}>—</span>)
                    : (userReferralRank?.rank
                        ? <span style={{ fontFamily:'Orbitron,sans-serif', fontWeight:700, fontSize: userReferralRank.rank >= 1000 ? 8 : userReferralRank.rank >= 100 ? 9 : 11, color:'rgba(255,255,255,0.7)', textAlign:'center', lineHeight:1.2 }}>#{userReferralRank.rank}</span>
                        : <span style={{ fontSize:16 }}>—</span>)
                  }
                </div>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <p style={{ fontFamily:'Rajdhani,sans-serif', fontSize:11, marginTop:1,
                    color: tab === 'points'
                      ? (meData?.in_top_100 ? 'rgba(255,215,0,0.7)' : 'rgba(255,255,255,0.3)')
                      : (userReferralRank ? 'rgba(255,215,0,0.7)' : 'rgba(255,255,255,0.3)') }}>
                    {tab === 'points'
                      ? (meData?.in_top_100
                          ? `Rank #${meData.rank}`
                          : meData?.rank
                            ? `#${meData.rank} · Not yet ranked`
                            : 'Not yet ranked')
                      : (userReferralRank?.rank
                          ? `Rank #${userReferralRank.rank}`
                          : 'Not yet ranked')
                    }
                  </p>
                </div>

                {/* Score — always show real points/refs even outside top 100 */}
                <div style={{ textAlign:'right', flexShrink:0 }}>
                  <p style={{ fontFamily:'Orbitron,sans-serif', fontWeight:700, fontSize:15, color:'white' }}>
                    {tab === 'points'
                      ? Number(meData?.points ?? 0).toLocaleString()
                      : Number(meData?.referral_count ?? userReferralRank?.referral_count ?? 0).toLocaleString()}
                  </p>
                  ...
                  {tab === 'points' && meData && (
                    <p style={{ fontFamily:'Space Mono,monospace', fontSize:8, color:'rgba(255,255,255,0.25)', marginTop:1 }}>
                      {Number(meData.referral_count ?? 0)} refs
                    </p>
                  )}
                </div>
