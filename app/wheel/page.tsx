...
export default function WheelPage() {
  ...
  const [multiResult,      setMultiResult]      = useState<{ basePrize: any; multipliedPrize: any; totals: { points: number; gems: number; pcedo: number }; multiplier: number; gemsSpent: number } | null>(null);
  ...
  const MULTIPLIERS = [5, 10, 15, 20] as const;

  const spinWithMultiplier = useCallback(async (multiplier: number) => {
    if (spinningRef.current || multiSpinning || autoRunning) return;
    ...
    try {
      const res = await WheelApi.spinMulti(multiplier);

      // Animate wheel to the single winning segment
      const winIndex    = res.segment_index as number;
      const extraSpins  = (8 + Math.floor(Math.random() * 4)) * 360;
      const targetAngle = 360 - (winIndex * SEGMENT_ANGLE + SEGMENT_ANGLE / 2);
      const totalRot    = rotationRef.current + extraSpins + (targetAngle - (rotationRef.current % 360) + 360) % 360;
      rotationRef.current = totalRot;
      ...
      setMultiResult({
        basePrize:      res.base_prize,
        multipliedPrize: res.multiplied_prize,
        totals:         res.totals,
        multiplier:     res.multiplier,
        gemsSpent:      res.gems_spent,
      });
      ...
      setSpinning(false);
      setMultiSpinning(false);
      const msg = e instanceof Error ? e.message : 'Multiplier spin failed';
      setSpinError(msg);
    }
  }, [multiSpinning, autoRunning, segments, pcedoEarned, applyUserPatch]);
  ...
  const canSpin = loggedIn && gems >= SPIN_COST && !spinning && !autoRunning && !multiSpinning;
    ...
    <main style={{ minHeight: '100vh', background: '#000', position: 'relative', overflow: 'hidden', WebkitTapHighlightColor: 'transparent' as any }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, padding: '24px 20px 100px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px' }}>
        ...
        <button onClick={() => spin()} disabled={!canSpin} style={{
          ...
          {spinning ? 'SPINNING...' : autoRunning ? `AUTO · ${autoRemaining} LEFT` : multiSpinning ? 'SPINNING...' : `SPIN · ${SPIN_COST} 💎`}
        </button>

        {/* ── Multiplier selector ─────────────────────────────────────────── */}
        {loggedIn && !autoRunning && !multiSpinning && (
          <div style={{ width: '100%', position: 'relative' }}>
            <button onClick={() => { setShowMultiMenu(p => !p); setShowAutoMenu(false); }} style={{
              width: '100%', padding: '12px 16px', borderRadius: 12,
              background: 'rgba(255,180,0,0.06)', border: '1px solid rgba(255,180,0,0.2)',
              color: 'rgba(255,200,80,0.7)', fontFamily: 'Rajdhani,sans-serif',
              fontWeight: 700, fontSize: 14, cursor: 'pointer',
              display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 16 }}>✖️</span> Multiplier Spin
              </span>
              <span style={{ fontFamily: 'Space Mono,monospace', fontSize: 10, color: 'rgba(255,200,80,0.4)' }}>
                {showMultiMenu ? '▲' : '▼'}
              </span>
            </button>
            ...
            {showMultiMenu && (
              <div style={{
                ...
                background: '#0f0f0f', border: '1px solid rgba(255,180,0,0.2)',
                borderRadius: 12, overflow: 'hidden', boxShadow: '0 8px 32px rgba(0,0,0,0.7)',
              }}>
                <p style={{ fontFamily: 'Space Mono,monospace', fontSize: 9, color: 'rgba(255,200,80,0.5)', letterSpacing: 2, padding: '10px 14px 6px' }}>
                  1 SPIN · LAND ON A REWARD · MULTIPLY IT ×N
                </p>
                {([5, 10, 15, 20] as const).map(n => {
                  ...
                  return (
                    <button key={n}
                      onClick={() => affordable && !spinning ? spinWithMultiplier(n) : undefined}
                      disabled={!affordable || spinning}
                      style={{
                        width: '100%', padding: '14px 16px',
                        background: 'rgba(255,255,255,0.02)',
                        border: 'none', borderTop: '1px solid rgba(255,255,255,0.06)',
                        color: affordable ? 'white' : 'rgba(255,255,255,0.25)',
                        cursor: affordable && !spinning ? 'pointer' : 'not-allowed',
                        display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                      }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <span style={{ fontFamily: 'Orbitron,sans-serif', fontWeight: 900, fontSize: 20, color: affordable ? 'rgba(255,200,80,0.95)' : 'rgba(255,255,255,0.2)' }}>
                          ×{n}
                        </span>
                        <div>
                          <p style={{ fontFamily: 'Rajdhani,sans-serif', fontWeight: 600, fontSize: 12, color: 'rgba(255,255,255,0.55)', margin: 0 }}>
                            reward multiplied ×{n}
                          </p>
                          <p style={{ fontFamily: 'Space Mono,monospace', fontSize: 8, color: 'rgba(255,255,255,0.25)', margin: 0, marginTop: 2 }}>
                            spin once · win big
                          </p>
                        </div>
                      </div>
          ...
          <div style={{ width: '100%', position: 'relative' }}>
            ...
            {showAutoMenu && (
              <div style={{
                ...
                <p style={{ fontFamily: 'Space Mono,monospace', fontSize: 9, color: 'rgba(255,255,255,0.3)', letterSpacing: 2, padding: '10px 14px 6px' }}>
                ...
                {[3, 5, 10, 20, 50].map(n => {
                  ...
                  return (
                    <button key={n}
                      onClick={() => affordable ? startAutoSpin(n) : undefined}
                      disabled={!affordable}
                      style={{
      ...
      {showMultiResult && multiResult && (() => {
        const mp = multiResult.multipliedPrize;
        const bp = multiResult.basePrize;
        const iconMap: Record<string, string> = { points: '⭐', gems: '💎', pcedo: '🪙', skin: '🎨' };
        return (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.92)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px', overflowY: 'auto' }}>
            <div style={{ width: '100%', maxWidth: '360px', background: '#0d0d0d', borderRadius: '24px', border: `1px solid ${mp?.color ?? 'rgba(255,180,0,0.3)'}55`, padding: '28px 22px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px', boxShadow: `0 0 60px ${mp?.color ?? 'rgba(255,180,0,0.2)'}33` }}>

              {/* Badge */}
              <div style={{ width: 80, height: 80, borderRadius: '50%', background: `radial-gradient(circle, ${mp?.color ?? '#FFE66D'}44, transparent)`, border: `2px solid ${mp?.color ?? '#FFE66D'}66`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 38 }}>
                {iconMap[mp?.type ?? 'points'] ?? '⭐'}
              </div>

              <p style={{ fontFamily: 'Space Mono,monospace', fontSize: 10, color: 'rgba(255,200,80,0.6)', letterSpacing: 3 }}>×{multiResult.multiplier} MULTIPLIER</p>

              {/* Big multiplied reward */}
              <p style={{ fontFamily: 'Orbitron,sans-serif', fontWeight: 900, fontSize: 26, color: mp?.color ?? '#FFE66D', letterSpacing: 2, textAlign: 'center', textShadow: `0 0 24px ${mp?.color ?? '#FFE66D'}` }}>
                {mp?.prize ?? '—'}
              </p>
              ...
              <div style={{ width: '100%', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: '12px 16px' }}>
                <p style={{ fontFamily: 'Space Mono,monospace', fontSize: 9, color: 'rgba(255,255,255,0.3)', letterSpacing: 2, marginBottom: 8 }}>HOW IT WAS CALCULATED</p>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, flexWrap: 'wrap' }}>
                  <div style={{ textAlign: 'center' }}>
                    <p style={{ fontFamily: 'Orbitron,sans-serif', fontWeight: 700, fontSize: 15, color: 'rgba(255,255,255,0.5)' }}>{bp?.prize}</p>
                    <p style={{ fontFamily: 'Space Mono,monospace', fontSize: 8, color: 'rgba(255,255,255,0.25)' }}>LANDED ON</p>
                  </div>
                  <span style={{ fontFamily: 'Orbitron,sans-serif', fontWeight: 900, fontSize: 18, color: 'rgba(255,200,80,0.7)' }}>×{multiResult.multiplier}</span>
                  <span style={{ fontFamily: 'Orbitron,sans-serif', fontWeight: 900, fontSize: 18, color: 'rgba(255,255,255,0.2)' }}>=</span>
                  <div style={{ textAlign: 'center' }}>
                    <p style={{ fontFamily: 'Orbitron,sans-serif', fontWeight: 900, fontSize: 15, color: mp?.color ?? '#FFE66D' }}>{mp?.prize}</p>
                    <p style={{ fontFamily: 'Space Mono,monospace', fontSize: 8, color: 'rgba(255,255,255,0.25)' }}>YOU GET</p>
                  </div>
                </div>
              </div>

              {/* Cost paid */}
              <p style={{ fontFamily: 'Space Mono,monospace', fontSize: 9, color: 'rgba(255,255,255,0.25)', letterSpacing: 1 }}>
                CHARGED: {multiResult.gemsSpent} 💎 ({SPIN_COST} × {multiResult.multiplier})
              </p>

              <button
                onClick={() => { setShowMultiResult(false); setMultiResult(null); }}
                style={{ width: '100%', padding: '16px', background: 'linear-gradient(135deg,rgba(255,180,0,0.9),rgba(255,120,0,0.9))', border: 'none', borderRadius: '14px', cursor: 'pointer', fontFamily: 'Orbitron,sans-serif', fontWeight: 900, fontSize: '14px', color: '#000', letterSpacing: '2px' }}>
                AWESOME! ×{multiResult.multiplier}
              </button>
            </div>
          </div>
        );
