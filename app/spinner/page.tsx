...
import { saveEnergyCache } from '@/context/AuthContext';
...
export default function SpinnerPage() {
  const { loggedIn, points, energy, adsWatched, cooldownSeconds: serverCooldownSeconds, syncSpinner, watchAd, persistEnergy, activeSkin, refreshFromServer } = useAuth();
  ...
  // ── Seed from server energy (authoritative) — no localStorage used ────────
  // refreshFromServer fetches clean server values. localStorage is intentionally
  // NOT used for energy or cooldown to prevent client-side tampering.
  useEffect(() => {
    if (!loggedIn) return;

    // Show values already in context while fresh fetch is in-flight
    energyRef.current     = energy;
    adsWatchedRef.current = adsWatched;
    localPtsRef.current   = points;
    seededRef.current     = true;
    setLocalEnergy(energy);
    setLocalAdsWatched(adsWatched);
    setLocalPoints(points);

    // Seed cooldown from server value already in context
    if (serverCooldownSeconds > 0) {
      setAdCooldownSecs(serverCooldownSeconds);
    }

    // Fetch fresh server values — overwrites any stale context values
    refreshFromServer().catch(() => {});
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [loggedIn]);

  // ── Sync refs when server energy arrives (after refreshFromServer resolves) ──
  // This ensures that if a user tampered with localStorage, the UI corrects itself
  // as soon as the authoritative server value lands in context.
  useEffect(() => {
    if (!loggedIn || !seededRef.current) return;
    // Only update downward — never let context push energy higher than what
    // the spinner already has (the spinner drains energy locally between syncs).
  ...
  // ── Sync cooldown from server when refreshFromServer resolves ─────────────
  // serverCooldownSeconds comes from /auth/me — completely independent of
  // localStorage, so clearing DevTools storage cannot bypass the cooldown.
  useEffect(() => {
    if (!loggedIn || !seededRef.current) return;
    if (serverCooldownSeconds > 0) {
      setAdCooldownSecs(serverCooldownSeconds);
